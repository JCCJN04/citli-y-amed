...
interface PublicPasswordProtectionProps {
  onAuthenticated: () => void;
}

const PublicPasswordProtection: React.FC<PublicPasswordProtectionProps> = ({ onAuthenticated }) => {
  const [password, setPassword] = useState('');
  const [error, setError] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  
  // Contraseña para la versión pública/demo
  const CORRECT_PASSWORD = 'BODA2026';

  useEffect(() => {
    const isAuthenticated = sessionStorage.getItem('wedding_public_authenticated');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (password.toUpperCase() === CORRECT_PASSWORD) {
        sessionStorage.setItem('wedding_public_authenticated', 'true');
        onAuthenticated();
      } else {
        setError(true);
        setPassword('');
    ...
    <div className="w-full h-screen bg-[#e5e3dd] flex items-center justify-center relative overflow-hidden">
      ...
      >
        <div className="bg-white/80 backdrop-blur-sm rounded-lg shadow-2xl p-8 md:p-12 border border-stone-200/50">
          ...
          >
            Versión de demostración — datos personales ocultos
          </motion.p>
        </div>
...
export default PublicPasswordProtection;
